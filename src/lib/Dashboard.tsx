...
const Dashboard = () => {
  ...
  const getMedicosDisponiveis = () => {
    ...
    Object.entries(dashboardData.medicosAlas).forEach(([ala, medicos]) => {
      medicos.filter(m => m.status === 'disponivel').forEach(medico => {
        total++;
        ...
        <div className="dashboard-grid">
          ...
          <div className="card medicos-online-card">
            ...
            <div className="medicos-online">
              {medicosDisponiveis.length > 0 ? medicosDisponiveis.map((medico, index) => {
                const iniciais = medico.nome.split(' ').map(n => n[0]).join('');
